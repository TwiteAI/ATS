...
import { Pencil, Trash2, Plus, Users, Calendar, FileText, BarChart2, ExternalLink, DollarSign, Briefcase } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard 
          ...
          value={getUpcomingInterviews()} 
          icon={<Calendar className="h-6 w-6 text-cyan-400" />} 
        />
        <StatCard 
          title="Pending Review" 
          value={getCandidatesByStatus('Pending')} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-gray-800 rounded-lg p-6 shadow">
          ...
          <div className="h-64 flex items-end justify-around">
            ...
            <ChartBar label="Interviews" value={getUpcomingInterviews()} color="rgb(20, 184, 166)" />
            <ChartBar label="Pending" value={getCandidatesByStatus('Pending')} color="rgb(13, 148, 136)" />
            <ChartBar label="Offered" value={getCandidatesByStatus('Offered')} color="rgb(5, 150, 105)" />
          </div>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-700">
          <thead className="bg-gray-900">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">CTC</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Interview</th>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {candidate.status ? (
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                        ${candidate.status === 'Pending' ? 'bg-yellow-900 text-yellow-200' : 
                          candidate.status === 'Interviewing' ? 'bg-blue-900 text-blue-200' : 
                          candidate.status === 'Offered' ? 'bg-green-900 text-green-200' : 
                          candidate.status === 'Rejected' ? 'bg-red-900 text-red-200' : 
